...
const Socket_Message = defineStore("Socket_Message", {
    ...
    actions: {
        storeLocally(
            id: string,
            content: LocalMessageContent | Array<LocalMessageContent>,
            userInfo: ChatUserInfo | Array<ChatUserInfo>,
            type: "front" | "back",
        ): number | undefined {
            ...
            if (type === "front") {
                this.$state.messages
                    .get(id)
                    ?.data?.unshift(...(content as Array<LocalMessageContent>));
            } else {
                this.$state.messages
                    .get(id)
                    ?.data?.push(...(content as Array<LocalMessageContent>));
            }
