<template>
    <div class="list-main">
        <ul class="daisy-menu bg-base-200 w-full p-0 [&_li>*]:rounded-none">
            ...
            <UserItem :description="extractTextContent(msg.content.text)" :userid="msg.info.id"
                :callback="changeSocketTarget" :avatar="msg.info.avatar" :name="msg.info.name" :index="msg.info.id"
                :online="checkUserInUsermap(msg.info.id)" v-for="msg of messageList" v-if="!loading">
            </UserItem>
        </ul>
    </div>
</template>
...
onMounted(async () => {
    ...
    const result = await window.socket.readMessageList() as any;
    console.log(result);
