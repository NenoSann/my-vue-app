...
const icon = nativeImage.createFromPath(
    "D:\\Web\\ElectronVueApp\\assets\\oie_icon.png",
);
let tray: Tray;
const createWindow = () => {
...
const createSocket = async function (
    name: string,
    _id: string,
    avatar: string,
) {
...
app.on("ready", () => {
    ...
    ipcMain.handle("socket:create", (event, name, _id, avatar) => {
        Socketio.getInstance(name, _id, avatar);
    });
    ipcMain.handle("socket:getUserMap", (event, name, _id, avatar) =>
        console.log(createSocket(name, _id, avatar)),
    );
    ipcMain.handle(
        "socket:privateMessage",
        async (_event, to: string, content: PrivateMessage) => {
    ...
    ipcMain.handle(
        "socket:groupMessage",
        (_event, to: string, content: GroupMessage) => {
            return new Promise<Boolean>((resolve, reject) => {
                Socketio.getInstance()
    ...
    ipcMain.handle(
        ...
        (
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    const res = await Socketio.getInstance().queryMessages(
                        userId,
                        targetId,
                        type,
                        limit,
    ...
    ipcMain.handle(
        ...
        (
            ...
            return new Promise<void>(async (resolve, reject) => {
                try {
                    await Socketio.getInstance().writeMessageList(
                        info,
                        type,
                        content,
                    );
    ...
    ipcMain.handle(
        "emoji:addEmoji",
        async (_event, md5: string, remoteAdd: string) => {
            try {
                ...
                const localAdd =
                    await Socketio.getInstance().SqlLiteController.saveRemoteFile(
                        "emoji",
                        remoteAdd,
                        fileName,
                    );
                if (localAdd) {
                    Socketio.getInstance().SqlLiteController.insertEmoji(
                        md5,
                        unixStamp,
                        remoteAdd,
                        localAdd,
